...
### Data and models for Discrepantie percentage
D <- read.table('../original_analysis/Discrep_data.txt',header = T)
saveRDS(D, file = "discrepantiePercentageData.rds")
storedData <- readRDS("discrepantiePercentageData.rds")
identical(D, storedData, ignore.environment = TRUE)
...
saveRDS(outm, file = "discrepantiePercentage.rds")
monkeyModel <- readRDS("discrepantiePercentage.rds")
identical(outm, monkeyModel, ignore.environment = TRUE)

#####
# calculate the discrepantie Model for boys
#####
D <- read.table('../original_analysis/Discrep2_data.txt',header = T)
Dj <- D[D$sekse == 0,]
saveRDS(Dj, file = "discrepantieDataBoys.rds")
storedData <- readRDS("discrepantieDataBoys.rds")
identical(Dj, storedData, ignore.environment = TRUE)
...
saveRDS(outj, file = "discrepantieModelBoys.rds")
monkeyModel <- readRDS("discrepantieModelBoys.rds")
identical(outj, monkeyModel, ignore.environment = TRUE)
...
saveRDS(Dm, file = "discrepantieDataGirls.rds")
storedData <- readRDS("discrepantieDataGirls.rds")
identical(Dm, storedData, ignore.environment = TRUE)
...
saveRDS(outm, file = "discrepantieModelGirls.rds")
monkeyModel <- readRDS("discrepantieModelGirls.rds")
identical(outm, monkeyModel, ignore.environment = TRUE)
