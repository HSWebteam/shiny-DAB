...
D<-read.table('Monkey2_Sample.dat',header=T)
...
outm<-gamlss(LOscore~cs(lftd,df=2),sigma.formula=~cs(lftd,df=1.3),nu.formula=~cs(lftd,df=1),tau.formula=~1,weights=psw,family=JSU,data=Dm)
summary(outm)
outj<-gamlss(LOscore~cs(lftd,df=2.2),sigma.formula=~lftd,nu.formula=~lftd,tau.formula=~lftd,weights=psw,family=JSU,data=Dj)
summary(outj)
...
plot(outm)
pdf('output/Monkey2_Sample_m.pdf')
...
dev.off()
plot(outj)
pdf('output/Monkey2_Sample_j.pdf')
...
bi<-function(x,y,z){
  if (z==1){
    if (trunc(x/365,0)==6){u<-c(y-1.96*.0590,y+1.96*.0590)}
    if (trunc(x/365,0)==7){u<-c(y-1.96*.0565,y+1.96*.0565)}
    if (trunc(x/365,0)==8){u<-c(y-1.96*.0568,y+1.96*.0568)}
    if (trunc(x/365,0)==9){u<-c(y-1.96*.0560,y+1.96*.0560)}
    if (trunc(x/365,0)==10){u<-c(y-1.96*.0558,y+1.96*.0558)}
    if (trunc(x/365,0)==11){u<-c(y-1.96*.0551,y+1.96*.0551)}
    if (trunc(x/365,0)==12){u<-c(y-1.96*.0542,y+1.96*.0542)}}
  if (z==2){
    if (trunc(x/365,0)==6){u<-c(y-1.96*.0580,y+1.96*.0580)}
    if (trunc(x/365,0)==7){u<-c(y-1.96*.0590,y+1.96*.0590)}
    if (trunc(x/365,0)==8){u<-c(y-1.96*.0595,y+1.96*.0595)}
    if (trunc(x/365,0)==9){u<-c(y-1.96*.0586,y+1.96*.0586)}
    if (trunc(x/365,0)==10){u<-c(y-1.96*.0576,y+1.96*.0576)}
    if (trunc(x/365,0)==11){u<-c(y-1.96*.0557,y+1.96*.0557)}
    if (trunc(x/365,0)==12){u<-c(y-1.96*.0583,y+1.96*.0583)}}
  if (u[1]<0){u[1]<-0}
  if (u[2]>1){u[2]<-1}
  u}
...
tscore<-function(x,y,z){
  ...
  if (z==1){
    ...
    if (trunc(x/365,0)==7){ts<-c(tr,tr-1.96*10*sqrt(1-.89),tr+1.96*10*sqrt(1-.89))}
    if (trunc(x/365,0)==8){ts<-c(tr,tr-1.96*10*sqrt(1-.84),tr+1.96*10*sqrt(1-.84))}
    if (trunc(x/365,0)==9){ts<-c(tr,tr-1.96*10*sqrt(1-.81),tr+1.96*10*sqrt(1-.81))}
    if (trunc(x/365,0)==10){ts<-c(tr,tr-1.96*10*sqrt(1-.82),tr+1.96*10*sqrt(1-.82))}
    if (trunc(x/365,0)==11){ts<-c(tr,tr-1.96*10*sqrt(1-.81),tr+1.96*10*sqrt(1-.81))}
    if (trunc(x/365,0)==12){ts<-c(tr,tr-1.96*10*sqrt(1-.85),tr+1.96*10*sqrt(1-.85))}}
  if (z==2){
    mu<-predict(outj,what='mu',newdata=newx,type='response',data=Dj)
    sigma<-predict(outj,what='sigma',newdata=newx,type='response',data=Dj)
    nu<-predict(outj,what='nu',newdata=newx,type='response',data=Dj)
    ...
    # from standard normal to T with mean 50 and sd 10 
    tr<-qnorm(pJSU(log(y/(1-y)),mu,sigma,nu,tau))*10+50 
    if (trunc(x/365,0)==6){ts<-c(tr,tr-1.96*10*sqrt(1-.88),tr+1.96*10*sqrt(1-.88))}
    if (trunc(x/365,0)==7){ts<-c(tr,tr-1.96*10*sqrt(1-.85),tr+1.96*10*sqrt(1-.85))}
    if (trunc(x/365,0)==8){ts<-c(tr,tr-1.96*10*sqrt(1-.81),tr+1.96*10*sqrt(1-.81))}
    if (trunc(x/365,0)==9){ts<-c(tr,tr-1.96*10*sqrt(1-.80),tr+1.96*10*sqrt(1-.80))}
    if (trunc(x/365,0)==10){ts<-c(tr,tr-1.96*10*sqrt(1-.79),tr+1.96*10*sqrt(1-.79))}
    if (trunc(x/365,0)==11){ts<-c(tr,tr-1.96*10*sqrt(1-.82),tr+1.96*10*sqrt(1-.82))}
